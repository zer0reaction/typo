...
void set_main_window_settings(sf::RenderWindow &main_window,
...
int main(int argc, char *argv[]) {
    ...
    auto new_word = [&]() {
        ...
        // Setting the string for the textbox (to show the typos and the word to
        // write)
    ...
    auto update_iteration = [&]() {
        ...
        // If there are no typos and the whole word is entered, we update the
        // word to a new one
        ...
        if (input_text.length() == 0)
            main_textbox.hide_cursor();
        else
            main_textbox.show_cursor();
    ...
    auto text_input_processing = [&](sf::Event event) {
        ...
        // If any other key is pressed (from a to z), we add it to the input
        // line
        // TODO: add programming stuff (){}[];:+-<> and numbers
        else if (letter_typed >= 97 && letter_typed <= 122)
            input_text += letter_typed;
    };
    ...
    while (main_window.isOpen()) {
        ...
        while (main_window.pollEvent(event)) {
            ...
            // If the window is resized, we update the view (for things not to
            // stretch)
