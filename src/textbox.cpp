...
void textbox::draw_widget(sf::RenderWindow &window) {
    ...
    for(int i = 0; i < text.length(); i++) {
        ...
        if(color_specification[i] == 'e') character.setFillColor(error_color);
        if(color_specification[i] == 'n') character.setFillColor(normal_color);
        if(color_specification[i] == 'b') character.setFillColor(blank_color);
    ...
    if(is_cursor_shown) {
        ...
        cursor.setPosition(position_x + cursor_position * character_spacing, position_y);
        cursor.setFillColor(cursor_color);
        cursor.setSize(sf::Vector2f(cursor_thickness, font_size + font_size / 5.0));
...
void textbox::set_font(std::string font_path, int font_size, int character_spacing) {
    if(!font.loadFromFile(font_path)) std::cout << "Error loading font " << font_path << std::endl;
...
void textbox::set_text(std::string text, std::string color_specification) {
    if(text.length() != color_specification.length()) {
        std::cout << "Text and color specification are not the same length! Nothing changed. " << text.length() << " " 
            << color_specification.length() << std::endl;
        return;
    }
    this -> text = text;
...
void textbox::set_cursor_position(int position) {
...
void textbox::set_cursor_color(sf::Color color) {
    cursor_color = color;
}
