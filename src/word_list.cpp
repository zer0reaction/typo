...
std::vector<word> word_list::get_best_for_typo(std::vector<int> &typos,
                                               int amount) {
...
#define BATCH_SIZE 500
    ...
    std::vector<word> sub = std::vector<word>(
        words.begin() + start, words.begin() + start + BATCH_SIZE);
    ...
    if (max_count == 0) {
        ...
        // this is not efficient and can cause lags, but i don't know how to
        // make it better yet
